...
const ProfilePage = ({ token }) => {
    ...
    if (file) {
      try {
        ...
        const response = await axios.post(
          'https://hyperchat-t.onrender.com/api/upload/profile-photo',
          formData,
          {
            headers: {
              Authorization: token,
  ...
  const toggleEditMode = async () => {
    ...
    if (isEditing) {
      try {
        await axios.put('https://hyperchat-t.onrender.com/api/users/me', user, {
          headers: { Authorization: token },
        });
  ...
  return (
    ...
    >
      <div className="w-full max-w-sm bg-white p-6 rounded-2xl shadow-xl relative">
        ...
        <h3 className="text-sm font-semibold" style={{ color: "#1C64F2" }}>EMPLOYEE DETAILS</h3>
        ...
        {notifications.map((notification) => (
          <div
            key={notification.id}
            className="fixed top-4 right-4  p-4 rounded-lg shadow-lg z-50  bg-green-600 text-white px-4 py-1"
          >
