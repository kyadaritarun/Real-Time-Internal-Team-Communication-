...
import { FaEdit, FaArrowLeft } from 'react-icons/fa'; // Added FaArrowLeft
...
const Settings = ({ token, isOpen, onClose, updateAuth }) => {
  ...
  const fetchUserProfile = async () => {
    try {
      const response = await axios.get('https://hyperchat-t.onrender.com/api/users/me', {
  ...
  const handlePhotoChange = async (e) => {
    ...
    if (file) {
      try {
        ...
        const response = await axios.post('https://hyperchat-t.onrender.com/api/upload/profile-photo', formData, {
          headers: {
            Authorization: token,
  ...
  const saveProfileChanges = async () => {
    try {
      const response = await axios.put('https://hyperchat-t.onrender.com/api/users/me', user, {
        headers: { Authorization: token },
      });
      setUser((prev) => ({ ...prev, ...response.data }));
      addNotification('Profile updated successfully');
      setIsEditing(false);
      setCurrentPage('initial'); // Return to main menu after saving
    } catch (error) {
  ...
  const handlePasswordChange = async () => {
    try {
      await axios.put('https://hyperchat-t.onrender.com/api/users/me', { password: newPassword }, {
        headers: { Authorization: token },
      });
          ...
          <div className="w-full max-w-md bg-white p-6 rounded-2xl shadow-xl">
            ...
            <div className="mt-6 space-y-3">
              <button
                ...
                className="w-full p-3 bg-green-500 hover:bg-green-600 rounded text-white"
              >
                Change Password
              </button>
            </div>
