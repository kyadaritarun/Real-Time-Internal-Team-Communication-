...
//       }
//     };
//   }, [token, privateKey, currentUserId]);
...
// Notification sound (place notification.mp3 in public/)
const notificationSound = new Audio(<audio src="/Notification.mp3" controls></audio>);
    ...
    useEffect(() => {
        ...
        socket.current.on("chatMessage", (msg) => {
            ...
            if (isPrivate && !msg.file && safeRender(msg.sender?._id || msg.sender) !== currentUserId) {
                ...
                // Only increment unread if not in the current chat
                if (selectedChat !== safeRender(msg.sender?._id || msg.sender)) {
                    setUnreadMessages((prev) => ({
                        ...prev,
                        [safeRender(msg.sender?._id || msg.sender)]: (prev[safeRender(msg.sender?._id || msg.sender)] || 0) + 1,
                    }));
                    notificationSound.play().catch((err) => console.error("Sound play error:", err));
                }
            }
        ...
        Promise.all([
            ...
            axios.get("https://kyadari-tarun-internal-chatbox.onrender.com/api/messages/last-messages", { headers: { Authorization: token } }),
            axios.get(`https://kyadari-tarun-internal-chatbox.onrender.com/api/users/${socket.current?.userId || currentUserId}`, { headers: { Authorization: token } }),
            axios.get("https://kyadari-tarun-internal-chatbox.onrender.com/api/messages/unread-counts", { headers: { Authorization: token } }),
        ])
            .then(([usersRes, groupsRes, lastMessagesRes, currentUserRes, unreadCountsRes]) => {
                setUsers(usersRes.data);
                setGroups(groupsRes.data);
                setCurrentUser(currentUserRes.data);
                const formattedTimes = lastMessagesRes.data.map((msg) => ({
                ...
                setLastMessageTimes(formattedTimes);
                setUnreadMessages(unreadCountsRes.data);
